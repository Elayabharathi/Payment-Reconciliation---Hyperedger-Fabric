...
async function main() {
  ...
  try {
    ...
    const credPath = path.join(
      fixtures,
      "/organizations/peerOrganizations/org4.example.com/users/User1@org4.example.com"
    );
    const certificate = fs
      .readFileSync(
        path.join(credPath, "/msp/signcerts/User1@org4.example.com-cert.pem")
      )
      .toString();
    const privateKey = fs
