...
class PaymentReconContract extends Contract {
    ...
    // async queryCheck(ctx, accountNumber, checkNumber) {
    // //    const checkDetails = await ctx.stub.getState(
    //         accountNumber,
    //         checkNumber
    //     ); // get the check details from chaincode state
    //     if (!checkDetails || checkDetails.length === 0) {
    //         throw new Error(`${checkNumber} does not exist`);
    //     }
    //     console.log(checkDetails.toString());
    //     return checkDetails.toString();
    // }
